...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    @Query("SELECT t FROM Ticket t WHERE (:search IS NULL OR :search = '' OR CAST(t.id AS STRING) LIKE CONCAT('%', :search, '%') OR LOWER(t.title) LIKE LOWER(CONCAT('%', :search, '%'))) AND (:status IS NULL OR :status = 'Todos' OR t.status = :status) AND (:priority IS NULL OR :priority = 'Todas' OR t.priority = :priority) ORDER BY t.createdAt DESC")
